...
/**
 * 根据字典类型查询字典数据
 * 参考：SysDictTypeServiceImpl.selectDictDataByType()
 */
@Cacheable(cacheNames = CacheNames.SYS_DICT, key = "#dictType")
@Override
public List<SysDictDataVo> selectDictDataByType(String dictType) {
    // Mapper 的 default 方法直接返回 List<SysDictDataVo>，无需手动转换
...
/**
 * 新增字典类型
 * 参考：SysDictTypeServiceImpl.insertDictType()
 */
...
### 4.3 CacheUtils.evict / CacheUtils.clear - 清除缓存

> **注意**：本项目字典删除和重置缓存使用编程式 `CacheUtils`，而非 `@CacheEvict` 注解。
...
/**
 * 批量删除字典类型
 * 参考：SysDictTypeServiceImpl.deleteDictTypeByIds()
 */
@Override
...
/**
 * 重置字典缓存数据
 * 参考：SysDictTypeServiceImpl.resetDictCache()
 */
@Override
...
// 字典缓存（Spring Cache 格式：cacheName::key）
"sys_dict::sys_user_sex"      // 字典数据（CacheNames.SYS_DICT）
"sys_dict_type::sys_user_sex" // 字典类型（CacheNames.SYS_DICT_TYPE）
...
@Cacheable(cacheNames = CacheNames.SYS_DICT, key = "#dictType")
public List<SysDictDataVo> selectDictDataByType(String dictType) {
    // Mapper 直接返回 List<SysDictDataVo>
    List<SysDictDataVo> dictDatas = dictDataMapper.selectDictDataByType(dictType);
    // 实际项目返回 null 让 Spring Cache 不缓存空值
    return CollUtil.isNotEmpty(dictDatas) ? dictDatas : null;
}
```

> **说明**：本项目字典查询缓存未命中时返回 `null`（Spring Cache 默认不缓存 null），
> 而非 `Collections.emptyList()`。如需缓存空值防穿透，可改为返回 `new ArrayList<>()`。
...
CacheUtils.evict(CacheNames.SYS_DICT, dictType);                  // 清除单个缓存
CacheUtils.clear(CacheNames.SYS_DICT);                            // 清除整组缓存
```
