...
> **⚠️ 重要声明**：本项目采用三层架构（Controller → Service → Mapper），无独立 DAO 层！如 `plus-ui/` 目录存在，则包含前端代码（Vue 3 + Element Plus）。
...
## 前端 Bug 排查（plus-ui 存在时适用）

> 当 `plus-ui/` 目录存在时，Bug 可能出现在前端。以下是前端常见问题排查指南。
...
├── 打开浏览器 DevTools (F12)
│   ├── Console 有红色错误？
│   │   ├── TypeError: Cannot read properties of undefined → 数据未初始化/接口未返回
│   │   ├── 404 Not Found → API 路径与后端不匹配
│   │   └── CORS error → 跨域配置问题
│   ├── Network 请求失败？
│   │   ├── 401 → Token 过期/未登录（检查 plus-ui/src/utils/auth.ts）
│   │   ├── 403 → 无权限（检查 v-hasPermi 与后端 @SaCheckPermission 是否一致）
│   │   ├── 500 → 后端异常（转后端排查）
│   │   └── 请求未发出 → 检查 baseURL 配置（VITE_APP_BASE_API）
│   └── 页面空白/样式异常？
│       ├── 路由未注册 → 检查后端菜单配置的 component 路径
│       ├── 组件导入错误 → 检查 import 路径
│       └── 样式冲突 → 检查 scoped 属性
└── 非报错类问题
    ├── 数据不显示 → 检查 v-model 绑定、响应式变量
    ├── 字典不显示 → 检查 useDict() 参数与 sys_dict_type 表
    ├── 按钮不显示 → 检查 v-hasPermi 权限标识
    └── 分页不工作 → 检查 PageQuery 参数名
```
...
| 问题 | 原因 | 解决方案 |
|------|------|---------|
| 页面空白无报错 | 路由的 component 路径错误 | 检查菜单管理中的组件路径是否匹配 `views/` 下的文件路径 |
| 字典标签不显示 | `useDict()` 的 dict_type 不存在 | 在系统管理→字典管理中检查字典类型是否已创建 |
| 按钮消失 | `v-hasPermi` 权限标识与后端不一致 | 对比前端 `v-hasPermi` 值与后端 `@SaCheckPermission` 值 |
| 雪花 ID 精度丢失 | JS Number 精度限制（>2^53） | 后端已配置 Long→String 序列化，检查前端类型定义是否用 `string` |
| 表单提交无反应 | 校验未通过但未提示 | 检查 `el-form` 的 `:rules` 和 `prop` 是否匹配 |
| 导出失败 | 响应类型未设置 blob | 确认请求配置 `responseType: 'blob'`，使用 `proxy?.download` |
| 登录后白屏 | 动态路由加载失败 | 检查后端 `/getRouters` 返回的 component 字段 |

### 前端排查工具

| 问题类型 | 工具 | 操作 |
|---------|------|------|
| API 请求 | Network 面板 | 检查 Request/Response 详情 |
| JS 错误 | Console 面板 | 查看错误堆栈 |
| 组件状态 | Vue DevTools | 检查组件 props/data/computed |
| Store 状态 | Vue DevTools → Pinia | 检查 store 中的 state 值 |
| 路由问题 | Vue DevTools → Router | 检查路由匹配和参数 |
| 样式问题 | Elements 面板 | 检查 CSS 计算值和覆盖 |
