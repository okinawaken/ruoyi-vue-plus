...
## 10. 前端工作流页面（plus-ui 存在时适用）

> 当 `plus-ui/` 目录存在时，工作流不仅是后端引擎，还需要前端页面配合实现完整的审批体验。
...
│   ├── leave/                        # 请假示例
│   ├── spel/                         # SpEL 配置
│   └── workflowCommon/              # 通用工作流 API
├── views/workflow/                    # 工作流页面
│   ├── category/index.vue           # 流程分类管理
│   ├── processDefinition/
│   │   ├── index.vue                # 流程定义列表
│   │   └── design.vue               # 流程设计器
│   ├── processInstance/index.vue    # 流程实例管理
│   ├── task/
│   │   ├── taskWaiting.vue          # 我的待办
│   │   ├── taskFinish.vue           # 我的已办
│   │   ├── taskCopyList.vue         # 抄送给我
│   │   ├── allTaskWaiting.vue       # 所有待办（管理员）
│   │   └── myDocument.vue           # 我的发起
│   ├── leave/
│   │   ├── index.vue                # 请假列表
│   │   └── leaveEdit.vue            # 请假表单
│   └── spel/index.vue               # SpEL 表达式管理
└── components/Process/               # 流程公共组件
    ├── approvalButton.vue           # 审批按钮组（通过/驳回/转办等）
    ├── approvalRecord.vue           # 审批记录展示
    ├── flowChart.vue                # 流程图组件
    ├── flowChartImg.vue             # 流程图图片版
    ├── submitVerify.vue             # 提交审核弹窗
    ├── processMeddle.vue            # 流程干预（加签/减签）
    └── MessageType.vue              # 消息类型组件
```

### 10.2 核心前端组件用法

#### 审批按钮组（approvalButton）
```vue
<template>
  <!-- 在业务详情页中引入审批按钮 -->
  <approval-button
...
<template>
  <!-- 显示流程审批历史 -->
  <approval-record :instance-id="instanceId" />
</template>
```
...
<template>
  <!-- 展示流程进度图 -->
  <flow-chart :instance-id="instanceId" />
</template>
```
...
**1. 发起流程**：在业务表单提交时调用工作流 API
```typescript
import { startWorkFlow } from '@/api/workflow/workflowCommon';

// 提交表单并发起流程
const handleSubmit = async () => {
  await startWorkFlow({
    flowCode: 'leave01',        // 流程编码
    businessId: form.value.id,  // 业务主键
    variables: {}               // 流程变量
  });
};
```

**2. 待办任务页面**：使用 `taskWaiting.vue` 作为参考
- 展示待办任务列表
- 点击办理 → 跳转到业务详情页 + 审批按钮
...
| 类型 | 位置 | 说明 |
|------|------|------|
| 请假表单（完整示例） | `plus-ui/src/views/workflow/leave/leaveEdit.vue` | 业务表单 + 流程集成 |
| 待办任务列表 | `plus-ui/src/views/workflow/task/taskWaiting.vue` | 任务查询与办理 |
| 流程设计器 | `plus-ui/src/views/workflow/processDefinition/design.vue` | 可视化流程设计 |
| 审批按钮组件 | `plus-ui/src/components/Process/approvalButton.vue` | 通过/驳回/转办按钮 |
| 审批记录组件 | `plus-ui/src/components/Process/approvalRecord.vue` | 审批历史展示 |
| 工作流通用 API | `plus-ui/src/api/workflow/workflowCommon/index.ts` | startWorkFlow 等 |
