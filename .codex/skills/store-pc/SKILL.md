...
export const useUserStore = defineStore('user', () => {
  ...
  return { token, login, getInfo, logout };
});
```
...
```typescript
// Store 中的 API 错误由全局响应拦截器处理（plus-ui/src/utils/request.ts）
// Store 方法本身不需要 try-catch，除非需要特殊处理
const fetchData = async () => {
  // 错误会被 request.ts 的响应拦截器统一处理（ElMessage 提示）
