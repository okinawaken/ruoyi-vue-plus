...
## 八、前端验证码与通知（plus-ui 存在时适用）

> 短信/邮件验证码的发送在后端完成，但前端需要配合实现验证码输入框、倒计时按钮等交互。

### 8.1 登录验证码（已有实现）

**后端接口**：`CaptchaController`（图形验证码，非短信）

前端登录页已内置图形验证码：
- 获取验证码图片：`GET /auth/code`
- 登录时携带：`code` + `uuid`
...
const sendSmsCode = async () => {
  ...
  // 2. 调用后端发送接口
  await sendSms({ phone: form.value.phone });
  // 3. 启动倒计时（60秒）
  countdown.value = 60;
  timer = setInterval(() => {
    countdown.value--;
...
系统通知（非短信/邮件）在前端通过以下方式展示：
- **系统公告**：`plus-ui/src/views/system/notice/index.vue`（CRUD 管理）
- **实时推送**：通过 WebSocket/SSE 推送消息到前端（参见 websocket-sse 技能）

### 8.4 注意事项

- 短信/邮件的发送逻辑**全部在后端**，前端只负责交互
- 验证码校验也在后端完成，前端仅传递 `code` 参数
- 倒计时是前端行为，不依赖后端返回
- 短信发送频率限制建议前后端同时实现（前端倒计时 + 后端 @RateLimiter）
