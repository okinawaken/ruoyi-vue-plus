...
## 10. 前端安全实现（plus-ui 存在时适用）

> 当 `plus-ui/` 目录存在时，安全不仅是后端的事，前端也需要配合实现完整的认证授权链路。

### 10.1 Token 管理

**文件位置**：`plus-ui/src/utils/auth.ts`
...
export function getToken(): string {
  return useStorage().local.get(TokenKey);
}

export function setToken(access_token: string) {
  return useStorage().local.set(TokenKey, access_token);
}

export function removeToken() {
  return useStorage().local.remove(TokenKey);
}
```

**关键点**：
- Token 存储在 localStorage（通过 useStorage Hook）
- TokenKey 为 `'Admin-Token'`
- 登录成功后 `setToken(access_token)`，登出时 `removeToken()`

### 10.2 请求拦截器（自动附加 Token）

**文件位置**：`plus-ui/src/utils/request.ts`
...
| 配置项 | 说明 | 示例 |
|--------|------|------|
| `isToken: false` | 跳过 Token 认证（公开接口） | 登录、验证码 |
| `repeatSubmit: false` | 前端防重复提交 | 表单提交 |
| `isEncrypt: 'true'` | 启用请求体加密（配合后端 @ApiEncrypt） | 密码传输 |
| `timeout: 50000` | 请求超时（50秒） | 全局默认 |

### 10.3 响应拦截器（401/403 处理）
...
**文件位置**：`plus-ui/src/permission.ts`
...
### 10.5 权限指令（v-hasPermi / v-hasRole）

**文件位置**：`plus-ui/src/plugins/auth.ts`、`plus-ui/src/utils/permission.ts`
...
**权限检查逻辑**：
- `v-hasPermi`：匹配 `useUserStore().permissions` 中的权限标识
- `v-hasRole`：匹配 `useUserStore().roles` 中的角色标识
- 通配符 `'*:*:*'` 表示超级管理员
- 超级管理员角色标识为 `'admin'`

### 10.6 前端安全检查清单

- [ ] 登录接口使用 `isEncrypt: 'true'` 加密密码？
- [ ] 敏感操作按钮添加了 `v-hasPermi` 权限控制？
- [ ] 路由配置了正确的 `permissions` 元数据？
- [ ] 401 响应正确触发重新登录？
- [ ] Token 过期时间与后端 Sa-Token 配置一致？
- [ ] 白名单路由仅包含公开页面？
- [ ] 前端权限标识与后端 `@SaCheckPermission` 值一致？
