...
**如果存在 plus-ui/**：
> 检测到前端项目 `plus-ui/`（Vue 3 + Element Plus + TypeScript），将同时生成后端 + 前端文档。
> 前端扫描范围：`plus-ui/src/views/`、`plus-ui/src/api/`、`plus-ui/src/store/modules/`

**如果不存在 plus-ui/**：
> 当前为纯后端项目，仅生成后端文档。如需开发前端页面，请创建 plus-ui 项目。
...
**提取规则**:
1. 提取 `// TODO:`、`// FIXME:`、`// HACK:` 注释（后端 Java 文件）
2. 提取 `<!-- TODO -->`、`// TODO`、`/* TODO */` 注释（前端 Vue/TS 文件，如 plus-ui/ 存在）
3. 记录完整的文件相对路径和精确行号
4. 提取 TODO 的完整描述文本
5. 智能分类：
...
### 3B. 前端代码扫描（plus-ui/ 存在时执行）

> 如果第一步检测到 `plus-ui/` 目录存在，**必须执行此步骤**。

#### 扫描范围

```bash
# 前端页面
plus-ui/src/views/**/*.vue        # 排除 framework 页面（login、redirect、error 等）

# 前端 API
plus-ui/src/api/**/*.ts

# 前端 Store
plus-ui/src/store/modules/*.ts

# 前端组件
plus-ui/src/components/**/*.vue
```
...
| 统计项 | 扫描路径 | 说明 |
|--------|---------|------|
| 页面总数 | `plus-ui/src/views/` | 统计 `.vue` 文件数（按模块分组） |
| API 模块数 | `plus-ui/src/api/` | 统计 API 目录数（system、workflow 等） |
| Store 模块数 | `plus-ui/src/store/modules/` | 统计 `.ts` 文件数 |
| 自定义组件数 | `plus-ui/src/components/` | 统计组件目录数 |

#### 前后端一致性检查

对每个后端业务 Controller，检查是否有对应的前端文件：

| 后端文件 | 对应前端文件 | 说明 |
|---------|-------------|------|
| `XxxController.java` | `views/[模块]/[功能]/index.vue` | CRUD 页面 |
| `XxxController.java` | `api/[模块]/[功能]/index.ts` | API 请求 |
| `XxxController.java` | `api/[模块]/[功能]/types.ts` | 类型定义 |

**前端完整度**：每个功能模块检查 3 个前端文件是否存在，计算覆盖率。

#### 输出到文档

在 `docs/项目状态.md` 中增加前端统计段：
