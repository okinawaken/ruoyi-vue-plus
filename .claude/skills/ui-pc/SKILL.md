...
路由由**后端动态生成**，通过 `/getRouters` 接口返回。前端通过 `permission.ts` 中的 `generateRoutes()` 动态注册。
...
|------|------|------|
| `path` | 路由路径 | `notice` |
| `component` | 组件路径（相对 views/） | `system/notice/index` |
| `perms` | 权限标识 | `system:notice:list` |
| `icon` | 菜单图标 | `message` |

**注意**：新增页面后必须在**系统管理→菜单管理**中配置对应菜单，否则路由不会生成。

### 国际化 (i18n)

国际化文件位置：`plus-ui/src/lang/`
