...
```yaml
# application.yml（实际配置见 ruoyi-admin/src/main/resources/application.yml）
tenant:
  # 是否开启多租户
  enable: true
...
> **注意**：`sys_dict_type` 和 `sys_dict_data` **不在排除列表中**，这两张表有 `tenant_id` 字段，受租户隔离，每个租户拥有自己的字典数据。

### 4.2 排除表说明

以下类型的表通常需要排除：
- **菜单表**：`sys_menu`，所有租户共享菜单结构
- **租户管理表**：`sys_tenant`、`sys_tenant_package`，租户自身的管理数据
- **角色关联表**：`sys_role_dept`、`sys_role_menu`，角色权限配置
- **用户关联表**：`sys_user_post`、`sys_user_role`，用户角色/岗位关联
- **客户端配置**：`sys_client`，OAuth 客户端配置
- **OSS 配置**：`sys_oss_config`，对象存储配置
- **代码生成表**：`gen_table`、`gen_table_column`（框架在 `PlusTenantLineHandler` 中自动排除）
...
```yaml
# application.yml（与 4.1 节一致，完整排除列表）
tenant:
  # 是否开启多租户
  enable: true
  # 排除表（不追加租户条件的表）
...
> **注意**：`sys_dict_type` 和 `sys_dict_data` 有 `tenant_id` 字段且**不在排除列表**中，属于租户隔离数据。
