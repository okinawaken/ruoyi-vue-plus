...
> **说明**：本项目是 RuoYi-Vue-Plus 全栈项目，后端采用三层架构（Controller → Service → Mapper）。
> **前端检测**：如果 `plus-ui/` 目录存在，则包含 PC 管理端前端代码（Vue 3 + Element Plus）。
...
## 前端代码结构（plus-ui，如目录存在）

> **检测规则**：如果 `plus-ui/` 目录存在 → 包含 PC 管理端前端代码（Vue 3 + Element Plus + TypeScript）
...
│   ├── system/                       # 系统模块 API
│   │   ├── notice/
│   │   │   ├── index.ts             # API 请求方法（list/get/add/update/del）
│   │   │   └── types.ts            # 类型定义（VO/Query/Form）
│   │   ├── user/
│   │   ├── role/
│   │   └── ...
│   └── demo/                         # 演示模块 API
...
每个 CRUD 功能需要生成 **3 个前端文件**：

| 文件 | 路径 | 说明 |
|------|------|------|
| API 请求 | `plus-ui/src/api/[模块]/[功能]/index.ts` | list/get/add/update/del 方法 |
| 类型定义 | `plus-ui/src/api/[模块]/[功能]/types.ts` | VO/Query/Form 接口定义 |
...
| 类型定义参考 | `plus-ui/src/api/system/notice/types.ts` |
