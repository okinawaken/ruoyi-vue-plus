...
如果技能涉及前端交互，SKILL.md 中应包含独立的前端章节：

```markdown
## 前端 XXX（plus-ui 存在时适用）

> 当 `plus-ui/` 目录存在时，XXX 不仅是后端功能，还需要前端配合实现完整体验。
...
**关键要求**：
- 前端代码示例基于项目实际文件（`plus-ui/src/` 下的真实组件）
- 全局类型（BaseEntity、PageQuery 等）无需 import，直接使用
- 使用 `<script setup lang="ts">` Composition API
- 权限用 `v-hasPermi`，字典用 `useDict()` + `<dict-tag>`
- 前端参考代码位置表必须指向真实文件
